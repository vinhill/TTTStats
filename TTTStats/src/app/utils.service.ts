...
export class UtilsService {
  ...
  private cmaps : {[key: string]: Colormap} = {
  ...
  randColor(): number[] {
    return [this.randInt(0, 255), this.randInt(0, 255), this.randInt(0, 255)];
  }
  ...
  getColormap(palette: string, num: number) : string[] {
    if (palette == "chartjs")
      return this.chartjsColormap(num);
    else 
      return this.steppedColormap(palette, num);
  }

  protected chartjsColormap(num: number) : string[] {
    var chartColors: string[] = [];
    for (let i = 0; i < num; i++) {
      chartColors[i] = this.toHex(chartjsColors[i] || this.randColor());
    }
    return chartColors
  }

  protected steppedColormap(palette: string, num: number) : string[] {
    if (!this.cmaps.hasOwnProperty(palette)) {
      throw new Error('Unknown palette: ' + palette);
    }
    var cmap = this.cmaps[palette];
       ...
       //Find where to get a color from a gradient [0-100]
        ...
        for (let stopNum = 0; stopNum < stops.length; stopNum++) {
          ...
          if (colorIdx === stopIdx) {
            ...
            chartColors[i] = this.toHex(cmap.get(stopIdx));
            break;
          }
            ...
            chartColors[i] = this.toHex(color);
            break;
          }
          // else continue
        }
