...
router.post("/parse", async function(req,res,next){
  let config = req.body.url;
  let date = req.body.date;
  if (!config || !date) {
    res.status(400).send("Post request has to contain config url and date.");
  }
  // TODO check if date is already present in DB
  let fetched = await fetch(`https://raw.githubusercontent.com/vinhill/vinhill.github.io/master/TTT/logs/${config}`);
